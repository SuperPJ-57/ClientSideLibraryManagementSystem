...
namespace ClientSideLibraryManagementSystem.Services
{
    public class TransactionService: ITransactionService
    {
        ...
        public async Task<IEnumerable<TransactionsEntity>> GetAllTransactionsAsync(string token)
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _httpClient.GetAsync("https://localhost:7084/api/Transactions");
            response.EnsureSuccessStatusCode();

            return await response.Content.ReadFromJsonAsync<IEnumerable<TransactionsEntity>>();
        }

        public Task<TransactionsEntity> GetTransactionByIdAsync(int transactionId)
        {
            throw new NotImplementedException();
        }
